...
**Not yet claimed:** statistical precision/recall on large *real* golden matters (the suites here are still synthetic), licensed reporter treatment, live-model rates *meeting the written bar across all three workflows*, and attorney-reviewed quality until Section B has an `attorney` pass.

**Section A status (2026-08-17):** closed-with-documented-residual — **not** a bar pass, **not** Harvey-level, **not** still-open for more prompt-example stacking. **Measured residual baseline:** `nyaya-matter-qa-v7`, `compare-summary-v4`, `gpt-4o-mini-2024-07-18` (Case Q&A fingerprint `fp_786821a2b4`, compare `fp_3cb74a061c`). Contradiction **fully meets** its numeric bar on that measured config (live 9: 21/21 × 3). Case Q&A and contract-compare **do not**. Further prompt-example stacking is deliberately paused: live 7–9 reshuffled failures as often as they fixed them, even though the magnitude of failure decreased (compare decoy FPR 50% → 0%).

**Answer thoroughness (`nyaya-matter-qa-v8`, 2026-08-17):** Case Q&A now asks for a complete attorney-facing `answer` when grounded/partial (direct answer, source support, uncertainties, next step only from a source gap). Insufficient refusals stay short. Research synthesis (`research-synthesis-v2`) asks for issue / rule / application / caveats in `conciseAnswer`. This is a UX completeness change, **not** a new worked example and **not** a live remasure. Live 9 residuals below remain the measured baseline.

Named residuals:

- **Case Q&A false-confidence 2.6% (bar: 0%).** Entirely `golden-false-rent-amount` 3/3. The model’s answer text is already a correct refusal (no “seven thousand” bait echo). The miss is `validateCitedAnswerAgainstPassages`: any retained valid cite coerces `insufficient` → `partial`, and live 9 kept `chunk_lease_term` (commencement/expiration; does not answer the rent question). This is a labeling upgrade, not the model asserting a false rent figure. **Candidate fix (not built; future scorer-only session):** when the parsed state is already `insufficient`, coerce to `partial` only if at least one retained cite’s passage plausibly bears on the question (content-token overlap with the question, excluding stopwords — not “any surviving cite”). If none bear, drop those cites so the existing empty-sources path keeps `insufficient` with zero sources (QA-04). Leave already-`partial` / `grounded` answers alone. Must still coerce QA-05 hedges (`golden-partial-hedge-indemnity`, `golden-partial-hedge-term`), whose cites do bear on the asked issue. Risk if built carelessly: synonym/paraphrase (a rent clause that never says “rent”) could fail to coerce a genuine incomplete answer, and the validator today does not receive the question. Do not add a fifth worked example for this.
- **Case Q&A hallucination still straddles &lt;5%.** Live 9 mean 4.4%; **max observed 5.3%**. Cite-acc mean 95.6% (range stays ≥90%); false-insufficient mean 1.8% (meets &lt;10%).
- **Contract-compare citation accuracy still misses ≥90%.** Live 9: 47.2% (41.7–53.8). Two named sources: golden-pair **“as is”** (real model omission, 3/3 since live 7; isolated `cc-live-material-as-is` still passes) and **`cc-live-mixed-term-labelled`** (1/3 on live 9: over-application of the isolated-decoy string `"No material changes detected."` on a mixed pair that contains a 2026→2027 term edit; #2/#3 passed). Compare decoy FPR on this config is **0/36**.
- **`golden-adv-near-miss-ninety-day-draft` is noisy**, not a prompt-version regression: 1/3 (live 6–7) → 3/3 (live 8) → 1/3 (live 9). Open item; do not chase it with another example.

Section B (attorney review) is **unchanged: not started.** It is now the only active blocker on any Harvey-level claim for these workflows.
...
**Open risks (accepted residuals, not a pass):** live 9 Case Q&A false-confidence 2.6% (validator coerce on `golden-false-rent-amount`, not a false-fact assertion); hallucination range straddles &lt;5% (max 5.3%); contract-compare citation accuracy 47.2% (golden-pair “as is” 3/3 since live 7; `cc-live-mixed-term-labelled` 1/3). `golden-adv-near-miss-ninety-day-draft` remains noisy (1/3–3/3). Prompt-example stacking is paused. Contradiction meets its bar on this config. Section B is not started. Bars not softened. See **Not yet claimed** above.
