/**
 * Wave 2AG — Finish MT, then CRITICAL_DEPTH historical high-court deepen.
 * Order: mont → iowa → me → miss → ark → sd → idaho → wyo → neb → ala → ky
 * target_max=45 (+~25 historical via CL_DATE_FILED_LTE). No invented intermediates.
 *
 * Usage: node scripts/run-wave2ag-depth-ingest.cjs [startCourt]
 */
"use strict";
...
const startCourt = (process.argv[2] || "mont").toLowerCase();
const root = path.join(__dirname, "..");
const outPath = path.join(root, "packages/research/corpus/reports/wave2ag-depth-ingest.json");

const PLAN = [
  { court: "mont", j: "MT", target: 45, batch: 8, dateLte: "2018-12-31", dateGte: "" },
  { court: "iowa", j: "IA", target: 45, batch: 8, dateLte: "2018-12-31", dateGte: "" },
  { court: "me", j: "ME", target: 45, batch: 8, dateLte: "2018-12-31", dateGte: "" },
  { court: "miss", j: "MS", target: 45, batch: 8, dateLte: "2018-12-31", dateGte: "" },
  { court: "ark", j: "AR", target: 45, batch: 8, dateLte: "2018-12-31", dateGte: "" },
  { court: "sd", j: "SD", target: 45, batch: 8, dateLte: "2018-12-31", dateGte: "" },
  { court: "idaho", j: "ID", target: 45, batch: 8, dateLte: "2018-12-31", dateGte: "" },
  { court: "wyo", j: "WY", target: 45, batch: 8, dateLte: "2018-12-31", dateGte: "" },
  { court: "neb", j: "NE", target: 45, batch: 8, dateLte: "2018-12-31", dateGte: "" },
  { court: "ala", j: "AL", target: 45, batch: 8, dateLte: "2018-12-31", dateGte: "" },
  { court: "ky", j: "KY", target: 45, batch: 8, dateLte: "2018-12-31", dateGte: "" },
];
...
function runOne(item) {
  ...
  return { exit: r.status, last, stderr: (r.stderr || "").slice(0, 500) };
}

const results = [];
console.log(JSON.stringify({ ok: true, wave: "2AG", plan: plan.map((p) => p.court), startCourt, featureAgents: "0" }));
...
for (const item of plan) {
  ...
  while (!done && rounds < 20) {
    ...
    results.push({ court: item.court, j: item.j, round: rounds, result: last, exit });
    console.log(
      JSON.stringify({
        court: item.court,
        round: rounds,
        status: last?.status,
        imported: last?.items_imported,
        apiCalls: last?.apiCalls ?? last?.api_calls,
      }),
    );
