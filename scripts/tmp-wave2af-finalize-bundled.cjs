...
// scripts/tmp-wave2af-finalize.cjs
var postgres = require_src();
var fs = require("fs");
var WAVE1 = ["LA", "DC", "NV", "TN", "MT"];
var TOP10_BEFORE = ["LA", "DC", "TN", "NV", "MT", "MS", "NM", "UT", "SD", "ID"];
...
async function jurisStats(sql, j) {
  ...
  `;
  const recentHeavy = Number(agg.cases || 0) > 0 && Number(agg.cases_last_3y || 0) / Math.max(1, Number(agg.cases || 0)) >= 0.7;
  return {
    j,
...
async function main() {
  ...
  try {
    ...
    for (const j of states) {
      ...
      const score = row.authorityDeficitTo101 * 8 + (row.intermediate === 0 && j !== "DC" ? 120 : 0) + (row.recentHeavy ? 80 : 0) + Math.max(0, 8 - row.distinct_years) * 15 + Math.min(120, row.citation.target_absent) * 0.4;
      ranked.push({
        ...row,
