...
// scripts/staging-cl-ingest-lean.ts
...
async function embedBatch(texts, apiKey) {
  ...
  const sorted = [...json.data].sort((a, b2) => a.index - b2.index);
...
async function insertChunks(sql, authorityId, versionId, content, apiKey) {
...
async function persistOne(sql, opinion, apiKey, opts) {
  ...
  if (existing.length > 0) {
    ...
    const latest = await sql`
      ...
      where authority_id = ${authorityId2}
    ...
    `;
    await insertChunks(sql, authorityId2, versionId2, opinion.content, apiKey);
    await sql`
      update legal_authorities
    ...
    if (opts.hasCitations) {
      await insertCitationEdges(sql, authorityId2, opinion.content);
    }
    return "new_version";
  }
...
function parseOpinion(sourceExternalId, searchHit, opinionRaw, retrievedAt, defaultClCourt, defaultMapped, unmappedCourts) {
  const merged = { ...searchHit, ...opinionRaw };
  const responseCourt = String(merged.court_id ?? "").trim().toLowerCase();
  ...
  const docket = merged.docket_number ? String(merged.docket_number) : merged.docketNumber ? String(merged.docketNumber) : null;
  if (!citation && !docket) {
    return {
      ok: false,
  ...
  const canonicalSourceUrl = absoluteUrl(merged.absolute_url) ?? absoluteUrl(merged.download_url) ?? null;
  return {
    ok: true,
    opinion: {
      sourceExternalId,
...
async function main() {
  ...
  const maxItems = proofMode ? 2 : Math.min(Math.max(Number.parseInt(process.env.CL_MAX ?? "20", 10) || 20, 1), 200);
  ...
  const unmappedCourts = /* @__PURE__ */ new Set();
  ...
  const sql = src_default(databaseUrl, { max: 5, ssl: "require" });
  try {
