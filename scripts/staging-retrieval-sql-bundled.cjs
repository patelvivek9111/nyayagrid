...
// scripts/staging-retrieval-sql.cjs
var postgres = require_src();
(async () => {
  const sql = postgres(process.env.DATABASE_URL, { ssl: "require", max: 1 });
  const cites = ["28 U.S.C. \xA7 1331", "29 CFR \xA7 541.100", "42 Pa.C.S. \xA7 5525", "Fed. R. Civ. P. 12"];
  const hits = {};
  for (const c of cites) {
    const rows = await sql`select citation, authority_type::text as t, authority_state, left(title,80) as title, canonical_source_url is not null as has_url from legal_authorities where source_provider='us-primary-corpus' and (citation ilike ${"%" + c.replace("\xA7", "%") + "%"} or title ilike ${"%" + c + "%"}) limit 3`;
    hits[c] = rows;
  }
  const ny = await sql`select citation, court_level, authority_state from legal_authorities where source_provider='us-primary-corpus' and authority_state='NY' and authority_type='case' limit 5`;
  const ca = await sql`select citation, court_level, authority_state from legal_authorities where source_provider='us-primary-corpus' and authority_state='CA' and authority_type='case' limit 5`;
  const chunks = await sql`select count(*)::int as n from legal_authority_chunks c join legal_authorities a on a.id=c.authority_id where a.source_provider='us-primary-corpus'`;
  const emb = await sql`select count(*)::int as n from legal_authority_chunks c join legal_authorities a on a.id=c.authority_id where a.source_provider='us-primary-corpus' and c.embedding is not null`;
  console.log(JSON.stringify({ ok: true, hits, ny, ca, chunks: chunks[0], embeddings: emb[0], fa: process.env.FEATURE_AGENTS }, null, 2));
