...
// scripts/staging-corpus-import-lean.ts
...
// scripts/staging-corpus-import-lean.ts
var PINNED = process.env.CORPUS_GIT_SHA?.trim() || "84f3bef1de66c766ea5da34a6b6185e783231375";
var BASE = `https://raw.githubusercontent.com/patelvivek9111/nyayagrid/${PINNED}/packages/research/corpus/bundles`;
...
async function loadAuthoritiesFromGithub() {
  const manifest = await fetchJson(`${BASE}/manifest.json`);
  const files = [
    ...(manifest.federalBundles ?? []).map((b2) => b2.bundleFile),
    ...manifest.states.map((s) => s.bundleFile)
  ];
  const authorities = [];
  for (const file of files) {
    const rows = await fetchJson(`${BASE}/${file}`);
...
async function countByType(sql) {
...
async function hasCurrentnessColumn(sql) {
  const rows = await sql`
    select 1 as ok
...
async function importOne(sql, auth, apiKey, hasCurrentness) {
  ...
  if (existing.length > 0) {
    ...
    `;
    await insertChunks(sql, authorityId2, versionId2, auth.content, apiKey);
    await sql`
...
async function runImport(sql, authorities, apiKey, hasCurrentness) {
  const summary = {
