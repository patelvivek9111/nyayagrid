...
// scripts/tmp-wave2ag-cite-recover.cjs
var postgres = require_src();
function softNorm(s) {
  if (!s) return null;
  return String(s).replace(/\u00a0/g, " ").replace(/\s+/g, " ").replace(/\bU\.\s*S\.\b/gi, "U.S.").replace(/\bF\.\s*(\d)d\b/gi, "F.$1d").replace(/\bF\.\s*Supp\.\s*(\d)?\b/gi, (_, n) => n ? `F. Supp. ${n}` : "F. Supp.").replace(/\bC\.\s*F\.\s*R\.\b/gi, "C.F.R.").replace(/\bU\.\s*S\.\s*C\.\b/gi, "U.S.C.").replace(/\s*§+\s*/g, " \xA7 ").trim();
}
async function main() {
  ...
  try {
    ...
    const add = (key, id) => {
      ...
      if (!k) return;
      if (!byNorm.has(k)) byNorm.set(k, /* @__PURE__ */ new Set());
      byNorm.get(k).add(id);
    };
    ...
    const PATTERNS = [
      { re: /\b(\d{1,2})\s+U\.?\s?S\.?\s?C\.?\s*§+\s*(\d[\w.\-]*)/gi, norm: (m) => `${m[1]} U.S.C. \xA7 ${m[2]}` },
      { re: /\b(\d{1,2})\s+C\.?\s?F\.?\s?R\.?\s*§*\s*(\d+(?:\.\d+)*)/gi, norm: (m) => `${m[1]} C.F.R. \xA7 ${m[2]}` },
      {
