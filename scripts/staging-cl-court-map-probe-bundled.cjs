// scripts/staging-cl-court-map-probe.ts
  ...
  for (const want of WANTED) {
    for (const needle of want.needles) {
      const found = courts.filter(
        (c) => String(c.name).toLowerCase().includes(needle) && (want.state === "MA" || want.state === "NY" || String(c.jurisdiction || "").toLowerCase().includes(want.state.toLowerCase()) || String(c.id).toLowerCase().startsWith(want.state.toLowerCase().slice(0, 2)) || true)
      );
