...
// scripts/staging-wave2c-citation-deepen.cjs
var postgres = require_src();
var PATTERNS = [
  { type: "statute", re: /\b(\d{1,2})\s+U\.?\s?S\.?\s?C\.?\s*§+\s*(\d[\w.\-]*)/gi, norm: (m) => `${m[1]} U.S.C. \xA7 ${m[2]}` },
  { type: "regulation", re: /\b(\d{1,2})\s+C\.?\s?F\.?\s?R\.?\s*§*\s*(\d+(?:\.[0-9A-Za-z\-]+)*)/gi, norm: (m) => `${m[1]} C.F.R. \xA7 ${m[2]}` },
  {
    type: "rule",
