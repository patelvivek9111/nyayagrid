...
function parseUsageRows(payload) {
  ...
  for (const row of root.current_usage ?? []) {
    ...
    const period = rate.includes("/min") ? "minute" : rate.includes("/hour") ? "hour" : rate.includes("/day") ? "day" : null;
    if (!period) continue;
  ...
  const minuteTarget = Math.max(
    1,
    Number.parseInt(process.env.CL_MINUTE_TARGET ?? "", 10) || Math.floor(windows.minute.limit * (14 / 15))
  );
  const hourTarget = Math.max(
    1,
    Number.parseInt(process.env.CL_HOUR_TARGET ?? "", 10) || Math.floor(windows.hour.limit * (140 / 150))
  );
  ...
  const fromMinute = Math.ceil(6e4 / Math.max(1, minuteTarget));
  const rateMs = Math.max(
    Number.parseInt(process.env.CL_RATE_MS ?? "", 10) || 0,
    fromMinute
  );
  const elevated = windows.minute.limit > 5 || windows.hour.limit > 50 || windows.day.limit > 125;
  return {
...
async function bootstrapQuotaPlan(apiKey) {
  try {
...
async function main() {
  ...
  const finish = async (payload, exitCode = 0) => {
    const sessionApiCalls = Number(cl.apiCalls) + (bootstrapUsage && quotaPlan ? 1 : 0);
    if (sql && job) {
      try {
    ...
    const out = {
      ...
      historicalApiCalls: Number.isFinite(historicalApiCallsBaseline) ? historicalApiCallsBaseline : job?.api_calls ?? null,
      sessionId,
      batchId,
  ...
  if (!courtVerify.ok) {
    const status2 = courtVerify.status === "rate_limited" ? cl.quotaExhausted ? "quota_paused" : "rate_limited" : courtVerify.status === "TRANSIENT_RETRY" ? "transient_retry" : courtVerify.status === "MAPPING_INVALID" ? "mapping_invalid" : "failed";
    await finish(
      {
        ok: status2 === "quota_paused" || status2 === "rate_limited" || status2 === "transient_retry",
        status: status2,
