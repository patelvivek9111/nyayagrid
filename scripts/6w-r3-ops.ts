#!/usr/bin/env npx tsx
/**
 * Phase 6W-R3 external-staging inventory and local probes.
 * Never prints secret values. Never overwrites 6W-R2 baselines.
 */
...
const outDir = join(repoRoot, "tmp", "6w-r3-ops");
...
const stagingDotenv = loadDotEnvKeys(join(repoRoot, ".env.staging"));

function present(key: string): boolean {
  const fromProcess = Boolean(process.env[key]?.trim());
  return fromProcess || dotenv.get(key) === true || stagingDotenv.get(key) === true;
}

function truthy(key: string): boolean {
  const raw = (process.env[key] ?? "").trim().toLowerCase();
  if (["1", "true", "yes", "on"].includes(raw)) return true;
  return dotenv.get(key) === true || stagingDotenv.get(key) === true;
}
...
record(
  "clerk.webhook",
  present("CLERK_WEBHOOK_SECRET") ? "AVAILABLE" : "MISSING",
  "CLERK_WEBHOOK_SECRET",
);
record(
  "inngest.eventKey",
  present("INNGEST_EVENT_KEY") ? "AVAILABLE" : "MISSING",
  "presence only; local/weak keys are not Cloud proof",
);
record(
  "inngest.signingKey",
  present("INNGEST_SIGNING_KEY") ? "AVAILABLE" : "MISSING",
  "presence only; local/weak keys are not Cloud proof",
);
record(
  "inngest.dev",
  truthy("INNGEST_DEV") ? "BLOCKED" : "PASS",
  "INNGEST_DEV must be unset/false for Cloud",
);
record(
  "redis.url",
  present("REDIS_URL") || present("REDIS_HOST") ? "AVAILABLE" : "MISSING",
  "app env Redis",
);
record(
  "s3.bucket",
  present("S3_BUCKET") ? "AVAILABLE" : "MISSING",
  "local MinIO rehearsal if endpoint is localhost",
);
record("database.url", present("DATABASE_URL") ? "AVAILABLE" : "MISSING", "presence only");
record(
  "malware.env",
  present("MALWARE_SCANNER") ? "AVAILABLE" : "MISSING",
  "development scanner is not staging proof",
);
record(
  "staging.baseUrl",
  present("STAGING_BASE_URL") ? "AVAILABLE" : "MISSING",
  "no staging host configured",
);
record("https.host", "MISSING", "no https:// staging host in env");
record("github.secrets", "EXTERNAL", "gh CLI unauthenticated; CI secret names not listed");
...
record(
  "feature.agents.production",
  productionAgents ? "CRITICAL" : "PASS",
  `production default agents=${productionAgents}`,
);

const base = (process.env.STAGING_BASE_URL ?? process.env.BETA_BASE_URL ?? "http://localhost:3000").replace(
  /\/$/,
  "",
);
const isHttpsStaging = /^https:\/\//i.test(base) && !/localhost|127\.0\.0\.1/i.test(base);
record(
  "probe.base",
  isHttpsStaging ? "AVAILABLE" : "BLOCKED",
  isHttpsStaging ? "HTTPS staging host configured" : "probe base is localhost — not staging proof",
);
...
async function main() {
  ...
  record(
    "http.live",
    live.status === 200 ? "PASS" : "BLOCKED",
    `GET ${base}/api/health/live → ${live.status || live.error} (localhost ≠ staging)`,
  );
  ...
  record(
    ...
    `GET ${base}/api/health/ready → ${ready.status} (development 200 is not staging proof; staging 503 expected until config complete)`,
  );
  ...
  record(
    ...
    `GET /api/v1/matters → ${matters.status} (local 401 is not Clerk session proof)`,
  );
  ...
  record(
    ...
    live.headers["content-security-policy"] && live.headers["x-frame-options"] && live.headers["x-content-type-options"]
      ? "PASS"
      : "NEEDS_WORK",
    `observed: ${headerNames.join(", ") || "none"}; HSTS only expected on production/staging HTTPS`,
  );
  ...
  try {
    ...
    record(
      "redis.docker",
      redis === "PONG" ? "PASS" : "NEEDS_WORK",
      "local docker Redis ping (not multi-instance staging proof)",
    );
  } catch {
  ...
  try {
    ...
    record(
      "malware.clamavContainer",
      clam === "healthy" ? "PASS" : "NEEDS_WORK",
      "local ClamAV container; app MALWARE_SCANNER must still be clamav for staging proof",
    );
  } catch {
  ...
  try {
    ...
    record(
      "storage.minio",
      minio === "healthy" ? "PASS" : "NEEDS_WORK",
      "local MinIO rehearsal; provider S3 remains EXTERNAL",
    );
  } catch {
  ...
  record("clerk.invites", "BLOCKED", "Clerk-managed invitations require a Clerk staging application");
  ...
  record("e2e.canary", "BLOCKED", "requires Clerk + Inngest Cloud + HTTPS host");
  record("coverage.canary", "BLOCKED", "requires authenticated staging Cases");
  ...
  const summary = {
    id: "BASELINE_6W_R3_OPS1",
    generatedAt: new Date().toISOString(),
    base,
    localhostIsNotStaging: true,
    technicallyDeployable: false,
    clerkBlocked: true,
    inngestCloudBlocked: true,
    httpsStagingBlocked: !isHttpsStaging,
    stopCondition: "B",
    checks,
  };
  ...
  writeFileSync(
    join(repoRoot, "benchmarks", "nyaya-bench", "baselines", "BASELINE_6W_R3_OPS1.json"),
    `${JSON.stringify(summary, null, 2)}\n`,
  );
  console.log(`wrote ${checks.length} checks to BASELINE_6W_R3_OPS1.json`);
}
