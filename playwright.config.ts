 ...
 *
 * Always start Next.js with this env (do not reuse an unrelated `npm run dev` process). Inngest on
 * :8288 is started the same way so upload → ingest does not depend on a forgotten worker terminal.
 */
const BASE_URL = process.env.PLAYWRIGHT_BASE_URL ?? "http://127.0.0.1:3000";
const APP_HEALTH_URL =
  process.env.PLAYWRIGHT_APP_HEALTH_URL ?? "http://127.0.0.1:3000/api/v1/health";
const INNGEST_DEV_URL = process.env.INNGEST_DEV?.includes("://")
...
export default defineConfig({
  ...
  webServer: [
    ...
    {
      command: "node scripts/e2e-inngest-dev.mjs",
