    ...
    try {
      ...
      setTurns((prev) => [
        ...prev,
        {
          ...
          needsMoreDocuments: Boolean(
            qa.needsMoreDocuments ?? validation.needsMoreDocuments,
          ),
        },
      ]);
    } catch (err) {
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center gap-2">
        ...
        <Link href={`/app/cases/${matterId}/chats`} className="text-xs text-accent underline">
          All Case chats
        </Link>
      </div>
      ...
      <ChatComposer
        ...
        onSubmit={send}
        busy={busy}
        caseChip={<CaseChip label={caseTitle} />}
        placeholder={`Ask about ${caseTitle}…`}
      />
      <SourceDrawer
        open={drawerOpen}
